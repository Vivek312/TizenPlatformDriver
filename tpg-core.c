...
#define COEFF(v, r) ((int)(0.5 + (v) * (r) * 256.0))
	...
	bool full = 1;//tpg->real_quantization == V4L2_QUANTIZATION_FULL_RANGE;
...
{
	unsigned step = 2;//V4L2_FIELD_HAS_T_OR_B(tpg->field) ? 2 : 1;
	unsigned div = step;
	unsigned first = 0;
...
{
	struct tpg_draw_params params;
	unsigned factor = 2;//V4L2_FIELD_HAS_T_OR_B(tpg->field) ? 2 : 1;
